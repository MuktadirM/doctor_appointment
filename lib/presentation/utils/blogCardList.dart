import 'package:doctor_appointment/presentation/models/blogCardModel.dart';

List<BlogCardModel> blogLsit = [
  BlogCardModel(
    imageUrl:
        "https://merriam-webster.com/assets/mw/images/article/art-wap-article-main/alt-5ae892611bf1a-5168-68b2575aab38f2c97ce8846381d07044@1x.jpg",
    blogTitle: 'Check your heart regularly',
    docName: 'Dr. Wang Shu',
    docTitle: 'Cardiac Specialist',
  ),

  BlogCardModel(
    imageUrl:
        "https://images.unsplash.com/photo-1505751172876-fa1923c5c528?ixid=MnwxMjA3fDB8MHxzZWFyY2h8Mnx8ZG9jdG9yfGVufDB8fDB8fA%3D%3D&ixlib=rb-1.2.1&w=1000&q=80",
    blogTitle: 'Check your heart regularly',
    docName: 'Dr. Wang Shu',
    docTitle: 'Family Specialist',
  ),
  BlogCardModel(
    imageUrl:
        "https://media.istockphoto.com/photos/doctor-holding-digital-tablet-at-meeting-room-picture-id1189304032?k=6&m=1189304032&s=612x612&w=0&h=SJPF2M715kIFAKoYHGbb1uAyptbz6Tn7-LxPsm5msPE=",
    blogTitle: 'Check your heart regularly',
    docName: 'Dr. Wang Shu',
    docTitle: 'Medicine Specialist',
  ),
  BlogCardModel(
    imageUrl:
        "https://merriam-webster.com/assets/mw/images/article/art-wap-article-main/alt-5ae892611bf1a-5168-68b2575aab38f2c97ce8846381d07044@1x.jpg",
    blogTitle: 'Check your heart regularly',
    docName: 'Dr. Wang Shu',
    docTitle: 'Cardiac Specialist',
  ),
  BlogCardModel(
    imageUrl:
        "https://images.unsplash.com/photo-1505751172876-fa1923c5c528?ixid=MnwxMjA3fDB8MHxzZWFyY2h8Mnx8ZG9jdG9yfGVufDB8fDB8fA%3D%3D&ixlib=rb-1.2.1&w=1000&q=80",
    blogTitle: 'Check your heart regularly',
    docName: 'Dr. Wang Shu',
    docTitle: 'Family Specialist',
  ),
  BlogCardModel(
    imageUrl:
        "https://media.istockphoto.com/photos/doctor-holding-digital-tablet-at-meeting-room-picture-id1189304032?k=6&m=1189304032&s=612x612&w=0&h=SJPF2M715kIFAKoYHGbb1uAyptbz6Tn7-LxPsm5msPE=",
    blogTitle: 'Check your heart regularly',
    docName: 'Dr. Wang Shu',
    docTitle: 'Medicine Specialist',
  ),
];
